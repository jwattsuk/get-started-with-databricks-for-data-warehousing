...
# MAGIC 1. Expand the **dbacademy > {schema_name} > Volumes**. (You can find your schema name in the output of the cell you just executed above.)
# MAGIC 1. Expand the **stream-source** directory. Notice that the directory contains two subdirectories: **customers** and **orders**.
...
# MAGIC
# MAGIC Steps:
# MAGIC 1. Open the [Pipelines user interface](/pipelines) (or use the **Jobs & Pipelines** option from the left sidebar and select the Jobs & pipelines tab).
# MAGIC 1. Click **Create** in the upper-right corner, and select **ETL pipeline** from the dropdown menu.
# MAGIC
# MAGIC **📌 Note:** A `Try the new ETL Pipeline` pop-up may appear on your screen. Please select **Maybe Later** to proceed with this course.
# MAGIC
...
# MAGIC - Under the **Details** tab of the materialized view, locate the table name (e.g., `dbacademy.schema_name.customer_order`).
