...
-- MAGIC
-- MAGIC You will now be presented with the Genie Space UI with the chat environment on the left and the settings and details on the right. With the **Configure** button at the top selected, click on **Settings**. (By default, the Configuration opens to **Data.**) Here you can edit the following information:
...
-- MAGIC
-- MAGIC 1. Navigate to **Dashboards** and select the dashboard you created during **Lesson - Creating AI-BI Dashboard in Databricks** (Retail Dashboard).
...
-- MAGIC 3. Click on **Publish** to open up the publishing dialog box.
...
-- MAGIC 7. Select the **Ask Genie** button embedded at the bottom. This opens a pop-up chat box on the right of the dashboard. You can use the kebab menu to access the settings to dock the Genie chat to the side of the screen.
