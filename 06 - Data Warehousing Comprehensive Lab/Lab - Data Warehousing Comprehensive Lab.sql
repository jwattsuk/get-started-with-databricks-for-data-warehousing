...
-- MAGIC 1. Using the [Catalog Explorer](/explore/data/dbacademy) user interface, upload the data file and create a table named **customers_ui** in your schema, using the file you just downloaded.
...
-- MAGIC 2. Use the **+ Add Data source** button to add datasets. 
-- MAGIC 3. Search for and select the **`customers_ui_gold`** table from `{DA.catalog_name}.{DA.schema_name}` and click **Confirm**. The table will appear in your dataset list.
-- MAGIC
...
-- MAGIC 4. Return to the **Untitled page** canvas tab and click **Add a visualization** at the bottom.
...
-- MAGIC     - **X axis:** Month
-- MAGIC     - **Y axis:**
-- MAGIC         - **Bar:** Total_Sales (Rename the Display name to **Total Sales Value**)
-- MAGIC         - **Line:** COUNT(`*`) (Rename the Display name to **Count of Sales Orders**)
-- MAGIC
...
-- MAGIC 2. Use the **Edit Draft** context button at the top of the dashboard display to make sure that you are looking at the **Draft** version.
-- MAGIC 3. Click the kebab menu (⋮) in the upper-right corner and select **Open Genie space (Draft)**.
